...
def calculate_user_penalty(df, exposures):
    ...
    # Corrected rules based on user requests:
    # We want exposure = 1.0 on dates where the user said it shouldn't be sold (noise exits)
    # and 0.0 on dates where they want to exit or be out.
    rules = [
        ("2016-01-26", 1.0), # user: "noise dual beli di tanggal 26 Jan 2016 ... harusnya nggak usah dijual"
        ("2016-08-09", 1.0), # user: "noise jual 9 Agt 2016 ... harusnya nggak usah dijual"
        ("2017-07-14", 1.0), # user: "14 Jul 2017 ... harusnya nggak usah dijual"
        ("2017-09-25", 1.0), # user: "25 Sep 2017 ... harusnya nggak usah dijual"
        ("2018-01-16", 0.0), # bear market drop
        ("2018-08-18", 0.0), # bear market drop
        ("2019-04-09", 1.0), # user: "noise jual di 9 Apr 2019 Harusnya itu nggak usah dijual"
        ("2019-07-30", 1.0), # user: "noise beli di 30 Jul 2019 ... harusnya nggak usah dijual"
        ("2020-03-10", 1.0), # user: "noise beli di 10 Mar 2020 ... harusnya nggak usah dijual"
        ("2020-05-23", 1.0), # user: "noise jual di 23 Mei 2020"
        ("2021-09-18", 1.0), # user: "noise beli di 18 Sep 2021"
        ("2021-09-25", 1.0), # user: "noise beli di 25 Sep 2021"
        ("2021-11-30", 1.0), # user: "noise beli di 30 Nov 2021"
        ("2023-08-01", 1.0), # user: "noise beli di 1 Agt 2023"
    ]
    ...
    # Range rules:
    ranges = [
        ("2017-01-25", "2017-01-28", 1.0), # user: Jan 2017 whipsaw - should have held (1.0)
        ("2021-02-08", "2021-02-12", 1.0), # user: Feb 2021 whipsaw - should have held (1.0)
        ("2021-10-03", "2021-10-11", 1.0), # user: Oct 2021 whipsaw - should have held (1.0)
        ("2020-02-16", "2020-03-03", 1.0), # user range rule
        ("2022-04-08", "2022-04-15", 0.0), # bear market range
        ("2023-03-13", "2023-03-15", 1.0), # user range rule
    ]
    ...
    # 2018 Bear Market trading penalty
...
def objective(x, df):
    ...
    use_ma_filter = bool(x[8] > 0.5)
    ma_period = max(10, int(round(x[9])))
...
def main():
    ...
    print("Running Differential Evolution with corrected rules for whipsaw exits...")
    ...
    with open("tmp/optimize_whipsaw_results.json", "w") as f:
        json.dump(output, f, indent=2)
    print("\nSaved output to tmp/optimize_whipsaw_results.json")
