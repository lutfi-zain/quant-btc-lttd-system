...
def calculate_user_penalty(df, exposures):
    ...
    for date_str, expected in rules:
        ...
        if ts in df.index:
            ...
            if exposures[idx] != expected:
                penalty += 5.0  # Softened from 15.0 to 5.0 per violation to keep weight balanced
    ...
    for start_str, end_str, expected in ranges:
        ...
        for idx in indices:
            if exposures[idx] != expected:
                penalty += 1.0  # Softened from 5.0 to 1.0 per day of violation
...
def objective(x, df):
    ...
    try:
        ...
        # Penalize Drawdown > 30%
        dd_penalty = max(0.0, -max_dd - 0.30) * 10.0
        ...
        # Balanced objective function: Maximize CAGR + 10 * Sharpe
        # Reward high return, high risk-adjusted return, penalize drawdown/trades/user violations
        score = cagr_val + 10.0 * sharpe - dd_penalty - trade_penalty - user_penalty
        return -score
...
def main():
    ...
    # bounds:
    # [entry_p, exit_p, score_entry, score_exit, cb_activate, cb_cooloff, rco_days, mhp_days, use_bear_override]
    bounds = [
        (5, 50),       # entry_p
        (2, 20),       # exit_p
        (0.20, 0.85),  # score_entry
        (-0.25, 0.45), # score_exit
        (-3.5, -1.0),  # cb_activate (was -3.0 to -0.5, let's allow deeper CB to prevent premature activation)
        (-1.0, 1.0),   # cb_cooloff
        (1, 15),       # rco_days
        (2, 20),       # mhp_days
    ...
    print("Running Differential Evolution to find optimal parameters...")
    ...
    print("\n" + "="*50)
    print("BEST BALANCED PARAMETERS FOUND:")
    print("="*50)
    ...
    print(f"  User Penalty Violations: {user_penalty:.1f} points")
    ...
    with open("tmp/optimize_sizing_improved_results.json", "w") as f:
        json.dump(output, f, indent=2)
    print("\nSaved output to tmp/optimize_sizing_improved_results.json")
