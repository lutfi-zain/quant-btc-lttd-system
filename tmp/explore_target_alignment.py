...
def main():
    ...
    # Let's inspect specific periods mentioned by the user
    periods = [
    ...
    for name, start, end in periods:
        ...
        # Print summary statistics and key dates
        print(f"  Close range: {sub['close'].min():.1f} - {sub['close'].max():.1f}")
        print(f"  Regimes present: {sub['regime'].unique()}")
        print(f"  Final score range: {sub['final_score'].min():.4f} to {sub['final_score'].max():.4f}")
        print(f"  Composite range: {sub['composite_value'].min():.4f} to {sub['composite_value'].max():.4f}")
        # Print dates where exposure changes
        diffs = sub["target_exposure"].diff().fillna(0.0) != 0
        changes = sub[diffs | (sub.index == sub.index[0]) | (sub.index == sub.index[-1])]
        print("  Key Exposure / State timeline:")
        for date, row in changes.iterrows():
            print(f"    {date.strftime('%Y-%m-%d')}: close={row['close']:8.1f} | regime={row['regime']:8} | score={row['final_score']:6.4f} | composite={row['composite_value']:6.4f} | exposure={row['target_exposure']:.1f}")
