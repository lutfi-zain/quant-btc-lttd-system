...
def test_binary_threshold_bull():
    ...
    assert calculate_target_exposure(0.51, "Sideways") == 0.0
    assert calculate_target_exposure(0.99, "Bear") == 0.0
    assert calculate_target_exposure(1.0, "Strong Bull") == 1.0


def test_binary_threshold_bear():
    assert calculate_target_exposure(0.499, "Bull") == 1.0
    assert calculate_target_exposure(0.0, "Sideways") == 0.0
    assert calculate_target_exposure(-0.5, "Bear") == 0.0
    assert calculate_target_exposure(0.2, "Weak Bull") == 1.0


def test_backward_compatibility_regime_ignored():
    # The regime argument is ignored/unknown and returns 0.0
    assert calculate_target_exposure(0.6, "INVALID_REGIME") == 0.0
