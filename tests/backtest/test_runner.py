...
def test_mock_execution_adapter():
    ...
    # Days 2 to 12 (hold due to score and MHP)
    for day in range(2, 13):
        date_str = f"2026-01-{day:02d}"
        res = adapter.run(date_str, 0.7, "BULL", {"BULL": 0.8, "BEAR": 0.1, "SIDEWAYS": 0.1})
        assert res["target_exposure"] == 1.0
        
    # Day 13: transition to SIDEWAYS, but stay in
    res2 = adapter.run("2026-01-13", 0.5, "SIDEWAYS", {"BULL": 0.0, "BEAR": 0.1, "SIDEWAYS": 0.9})
    assert res2["target_exposure"] == 1.0
    assert res2["regime"] == "SIDEWAYS"
    assert res2["transition_occurred"] is True
    assert len(adapter.transitions) == 1
    
    # Day 14: exit
    res3 = adapter.run("2026-01-14", -0.5, "SIDEWAYS", {"BULL": 0.0, "BEAR": 0.1, "SIDEWAYS": 0.9})
    assert res3["target_exposure"] == pytest.approx(0.0)
    assert res3["regime"] == "SIDEWAYS"
    assert res3["transition_occurred"] is False
    assert len(adapter.transitions) == 1
    assert adapter.transitions[0]["from_regime"] == "BULL"
    assert adapter.transitions[0]["to_regime"] == "SIDEWAYS"
