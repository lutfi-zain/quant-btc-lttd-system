...
class BRKDataFetcher:
    ...
    def fetch_latest(self, series_name: str) -> dict:
        ...
        stamp = datetime.strptime(stamp_str, "%Y-%m-%dT%H:%M:%SZ").replace(tzinfo=timezone.utc)
    ...
    def align_with_ohlcv(self, brk_df: pd.DataFrame, ohlcv_df: pd.DataFrame) -> pd.DataFrame:
        """
