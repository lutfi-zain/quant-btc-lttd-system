# Non-Goals
- Computation or recalculation of the LTTD Final Score, Indicator Score, or Regime (handled entirely by Python Layer 4 and Layer 5).
- Direct exposure to raw Technical Indicators or On-Chain Metrics unless serialized as part of the ensemble metadata.
- Writing data, mutating state, or managing Walk-Forward Optimization (WFO) backtest artifacts.
- Modifying the SQLite database (all SQLite connections MUST be strictly read-only).
