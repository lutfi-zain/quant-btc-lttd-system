...
## Non-Goals
- Non-linear dimensionality reduction (e.g., t-SNE, UMAP). The system strictly uses linear PCA to preserve interpretability.
- Applying PCA directly to raw OHLCV price data. It is only applied to the calculated Technical Indicator and On-Chain Metric values.
