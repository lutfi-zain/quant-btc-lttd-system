...
## Non-Goals
- Real-time external alerting (e.g., sending an email or Telegram message upon transition is out of scope).
- Logging every single daily bar when the regime does NOT change (only transitions are explicitly logged at this high verbosity).
