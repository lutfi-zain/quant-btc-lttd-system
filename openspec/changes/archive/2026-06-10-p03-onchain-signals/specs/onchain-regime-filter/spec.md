## Non-Goals
- Using On-Chain Metrics as execution triggers directly (they are strictly Regime filters).
- Incorporating bounded on-chain ratios directly into the HMM observation matrix, which would destabilize Gaussian emission assumptions.
- Overriding the `SIDEWAYS` state; filters primarily scale down `BULL` exposure during euphoric extremes.
