## Non-Goals
- Using the brk-client to fetch high-frequency (minute/hourly) data; the system focuses purely on daily On-Chain Metrics.
- Fetching Long-Term Holder (LTH) metrics which are outside the scope of the 120-350 day LTTD trend direction horizon.
- Developing custom REST clients (we strictly use the provided `brk-client` Python library).
